package dev.yuruni.raycastclient.gui;
...
public class ClickGUI extends MinecraftHUDGUI {
    ...
    public ClickGUI() {
        ...
        gui.addHUDComponent(TargetHUD.getComponent(), TargetHUD.getToggle(), animation.get(), theme, BORDER);
        ...
        IntFunction<IResizable> resizable=width->new IResizable() {
            Dimension size=new Dimension(width,160);
        ...
        // Single Panel
        IComponentAdder singlePanelAdder=new SinglePanelAdder(gui,new Labeled("RaycastClient Menu",null,()->true),theme,new Point(10,10),WIDTH*Category.values().length,animation,()->ClickGUIModule.layout.getValue()==ClickGUIModule.Layout.SinglePanel,"singlePanel") {
            @Override
            protected IResizable getResizable (int width) {
                return resizable.apply(width);
            }
    ...
    private class ThemeSelector implements IThemeMultiplexer {
        ...
        public ThemeSelector (IInterface inter) {
            ...
            addTheme(ClickGUIModule.Theme.Rainbow,new RainbowTheme(new ThemeScheme(ClickGUIModule.Theme.Rainbow),()->ignoreDisabled.getValue(),()->buttonRainbow.getValue(),()->rainbowGradient.getValue(),9,3,": "+Formatting.GRAY));
            addTheme(ClickGUIModule.Theme.Impact,new ImpactTheme(new ThemeScheme(ClickGUIModule.Theme.Impact),9,4));
        }

        @Override
        public ITheme getTheme() {
            return themes.getOrDefault(ClickGUIModule.theme.getValue(),themes.get(ClickGUIModule.Theme.Clear));
        }
